...
function isMediaFile(filePath: string): boolean {
    const mediaExtensions = ['.png', '.jpg', '.jpeg', '.gif', '.svg', '.mp3', '.mp4', '.pdf', 'jff','.txt'];
    ...
    try {
        ...
        if (workspaceFolders) {
            for (const folder of workspaceFolders) {
                const excludePattern = new vscode.RelativePattern(folder, '**/{node_modules,.git,dist,.vscode,*.png,*.jpg,*.jpeg,*.gif,*.bmp,*.svg}');
            ...
            vscode.window.showInformationMessage(`File paths written to ${filesFilePath}`);
        }
    } catch (error) {
...
export async function processChangedFiles() {
    if (changedFiles.size > 0) {
        ...
        const filesData = await Promise.all(Array.from(changedFiles).map(async filePath => {
            ...
            try {
                ...
                vscode.window.showInformationMessage(`file size: `, (Buffer.byteLength(content, 'utf-8')).toString());
                // Check if content size exceeds the maximum allowed size
                if (Buffer.byteLength(content, 'utf-8') > MAX_FILE_SIZE_BYTES) {
                    // File content exceeds the size limit
                    vscode.window.showWarningMessage(`${absolutePath} exceeds the maximum allowed size (1 MB). Skipping.`);
        ...
        vscode.window.showInformationMessage(`Changed files: ${JSON.stringify(validFilesData)}`);
...
function handleFileDeletion(uri: vscode.Uri, context: vscode.ExtensionContext) {
    ...
    vscode.window.showInformationMessage(`File ${filePath} has been deleted and its state has been removed.`);
}
